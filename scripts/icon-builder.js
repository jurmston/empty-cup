...
const includedIcons = iconList.map(snakeToPascal).reduce((iconsWithVariants, icon) => {
...
async function worker({ progress, svgPath, options, renameFilter, template }) {
  ...
  if (includedIcons.indexOf(componentName) === -1) {
    return
  }
...
export async function handler(options) {
  ...
  const [svgPaths, template] = await Promise.all([
    ...
    fse.readFile(path.join(__dirname, 'template-svg-icon.js'), {
      encoding: 'utf8',
    }),
  ]);
  ...
  // Override since we have no legacy files.
  // const legacyFiles = []
